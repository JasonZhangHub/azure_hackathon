...
def app():
    ...
    type_data = st.selectbox(
        "Number of COVID-19 cases or deaths by country (You can click on the legend labels to select countries to compare)",
    ...
    country_death_cases = casedata.groupby(['countriesAndTerritories'])[['cases', 'deaths']].aggregate(
        np.sum).reset_index()
    ...
    def create_dataframe_top(queries, country):
        ...
        if len(fulldata) > 0:
            fulldata = fulldata.drop_duplicates(subset='url').sort_values(by='publishedAt',
    ...
    def country_data_codes():
        ...
        country_options = ['ae', 'ar', 'at', 'au', 'be', 'bg', 'br', 'ca', 'ch', 'cn', 'co', 'cu', 'cz', 'de', 'eg',
                           'fr',
                           'gb', 'gr', 'hk', 'hu', 'id', 'ie', 'il', 'in', 'it', 'jp', 'kr', 'lt', 'lv', 'ma', 'mx',
                           'my',
    ...
    st.markdown(f"<center> <h1> Most Recent COVID-19 News Headlines from {country_name}</h1></center>",
                unsafe_allow_html=True)
